...
const Dashboard: React.FunctionComponent = () => {
  ...
  const onClaimDevice = async () => {
    try {
      await claimDevice(deviceIdValue, auth.userData?.access_token);
  ...
  const onReleaseDevice = async(deviceId) => {
    try {
      await releaseDevice(deviceId, auth.userData?.access_token);
      ...
      claim = (
        <React.Fragment>
          ...
          <Modal
            ...
            actions={[
              <Button key="confirm" variant="primary" onClick={onClaimDevice} isDisabled={validated !== 'success'}>Claim</Button>,
              <Button key="cancel" variant="link" onClick={handleModalToggle}>Cancel</Button>
            ]}
            >
            <Form id="claim-device-modal">
              <FormGroup
                label="Device ID"
                isRequired
