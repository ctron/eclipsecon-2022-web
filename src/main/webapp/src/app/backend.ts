...
const useGameService = (): [Service<DeviceClaim>, DispatchWithoutAction] => {
  ...
  useEffect(() => {
    fetch( API_BASE + '/api/v1alpha1/deviceClaims', {
      headers: new Headers({
...
const claimDevice = async (deviceId: string, accessToken?: string): Promise<Response> => {
  const url = API_BASE + '/api/v1alpha1/deviceClaims?' + new URLSearchParams({
    deviceId
  });
...
export {useGameService, claimDevice, releaseDevice};
